...
// ✅ قبل كل request — ضيف التوكن من localStorage
...
// ✅ لو الـ accessToken منتهي أو السيرفر رجع 401، جدده تلقائيًا
  ...
  async (error) => {
    if (error.response?.status === 401 && !error.config._retry) {
      ...
      try {
        ...
        // جرب نفس الطلب تاني بالتوكن الجديد
